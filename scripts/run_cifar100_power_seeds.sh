#!/usr/bin/env bash
# run_cifar100_power_seeds.sh  –  2 runs: seed 2,3 cho ô cifar100/power/eta1=0.01/R=5
# Chạy: bash scripts/run_cifar100_power_seeds.sh
...
# ═══════════════════════════════════════════════════════════════════════════════
echo -e "${CYAN}════════════════════════════════════════════${NC}"
echo -e "${CYAN}  cifar100 / power / eta1=0.01 / R=5${NC}"
echo -e "${CYAN}  seed 2, 3  |  MAX_JOBS=$MAX_JOBS${NC}"
echo -e "${CYAN}════════════════════════════════════════════${NC}"

for seed in 2 3; do
    submit_job "seed" ablation \
...
# ═══════════════════════════════════════════════════════════════════════════════
echo -e "${CYAN}════════════════════════════════════════════${NC}"
echo -e "${CYAN}  eval_acc_final  (cifar100/power/eta1=0.01)${NC}"
echo -e "${CYAN}════════════════════════════════════════════${NC}"
...
for s in (1, 2, 3):
    ...
    if os.path.isfile(f):
        d = json.load(open(f))
        print(f"  seed {s}:  eval_acc_final = {d['eval_acc_final']:.2f}%")
    else:
